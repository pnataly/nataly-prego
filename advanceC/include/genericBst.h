 ...
 * @params _destroyer : A function to destroy the data in the tree.
 */
void  BSTree_Destroy(BSTree** _tree, void (*_destroyer)(void*));

/* 
 * @brief Add an element to tree if is not already there.
 * Insert element to binary tree, using the tree's comparison function (_less).
...
/* 
 * @brief Search the first element for which the given predicate returns 1.
 ...
 * @param _context : Predicate parameter.
...
/*
 * @brief Get itertator to the next element from current iterator by in-order.
 * complexity: O(logn)
 *
 * @params _itr : A tree iterator
 * @returns an iterator pointing at the next element after _itr or end iterator.
 */
BSTreeItr BSTreeItr_Next(BSTreeItr _itr);

/*
 * @brief Get itertator to the previous element from current iterator by in-order.
 *
 * @param _itr : A tree iterator.
 * @return an iterator pointing at the previous element or to the beginning if _itr points to the beginning.
 */
BSTreeItr BSTreeItr_Prev(BSTreeItr _itrr);

/*
 * @brief Removes element from tree.
 * @details Remove element pointed to by _itr and rearranges the tree so that it maintains binary search tree arrangement
 ...
 * time complexity O(1).
 * We do not provide a BSTreeItr_Put(it, data) function.
